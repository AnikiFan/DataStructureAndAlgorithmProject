...
template <typename T>
inline Heap<T>::Heap(T *begin, T *end, bool (*compare)(const T &, const T &))
    : Vector<T>(begin, end), cmp(compare), heapsz{end - begin}
{
}
...
inline void Heap<T>::shrink()
{
    if (!heapsz)
    {
        throw std::underflow_error("Heap::shrink");
    }
    heapsz--;
    this->swap(0, heapsz);
    return;
...
inline void Heap<T>::maintain(long long i)
{
    ...
    if (i < 0 || i >= heapsz)
    {
        throw std::invalid_argument("Heap::maintian");
    };
    while (lchild(i) != null || rchild(i) != null)
    {
        ...
        {
            if (
                cmp(Vector<T>::elem[i], Vector<T>::elem[lchild(i)]) &&
                (cmp(Vector<T>::elem[rchild(i)], Vector<T>::elem[lchild(i)]) ||
                 (!cmp(Vector<T>::elem[rchild(i)], Vector<T>::elem[lchild(i)]) && !cmp(Vector<T>::elem[lchild(i)], Vector<T>::elem[rchild(i)]))))
            {
                // cout<<"case 3"<<endl;
                this->swap(i, lchild(i));
                i = lchild(i);
